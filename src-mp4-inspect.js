...
function parseAvcC(bytes, box) {
  ...
  for (let i = 0; i < numSps && o + 2 <= box.end; i++) {
    ...
    cfg.spsOffsets.push(o);                 // start of the SPS NAL, header byte included
...
function parseTrack(bytes, trak, movieTimescale) {
  ...
  if (isVideo) {
    ...
    if (vse.boxes.avcC) {
      ...
      // Byte offsets (relative to the moov payload) of every place the level is
      // written. Used by the in-browser level rewrite; see buildLevelPatch.
      t.levelByteOffsets = [vse.boxes.avcC.start + 3].concat(cfg.spsOffsets.map((o) => o + 3));
...
function simplifyRatio(w, h) {
  ...
  const g = gcd(a, b) || 1;
  const sw = a / g, sh = b / g;
  // 16:9 and 4:3 are useful; 427:240 is not. Fall back to the decimal form
  // the industry actually writes when the ratio does not reduce cleanly.
  if (sw > 50 || sh > 50) return `${(a / b).toFixed(2)}:1`;
  return `${sw}:${sh}`;
}
...
export async function inspect(reader, opts = {}) {
  ...
  result.moovOffset = moovBox.dataOffset;
...
/* ------------------------------------------------------------------ *
 * In-browser level rewrite                                            *
 * ------------------------------------------------------------------ */

/**
 * Work out which bytes have to change to redeclare the H.264 level.
 *
 * The level appears twice in an MP4: AVCLevelIndication at byte 3 of the
 * avcC box, and level_idc at byte 3 of each SPS NAL stored inside it. Both
 * are single bytes and neither changes length, so the rewrite is a splice
 * rather than a remux. Nothing else in the file is touched.
 *
 * Only valid for avc1 (parameter sets in the sample entry). avc3 and hev1
 * carry them in-band in mdat, which would mean walking every sample.
 *
...
export function buildLevelPatch(info, levelName) {
  ...
  if (v.codec.fourcc !== 'avc1') return null;

  const entry = H264_LEVELS.find((l) => l.name === String(levelName));
  if (!entry) return null;
  const idc = entry.name === '1b' ? 11 : Math.round(entry.level * 10);
  if (idc < 10 || idc > 255) return null;

  return v.levelByteOffsets.map((rel) => ({ offset: info.moovOffset + rel, byte: idc }));
}

/**
 * Apply a patch to a File or Blob without reading it into memory.
 *
...
export function applyPatch(file, patch) {
  ...
  let cursor = 0;
  for (const op of ops) {
    if (op.offset < cursor) return null;            // overlapping, refuse
    if (op.offset > cursor) parts.push(file.slice(cursor, op.offset));
    parts.push(new Uint8Array([op.byte]));
    cursor = op.offset + 1;
  }
...
export function rewriteLevel(file, info, levelName) {
  ...
  if (!blob) return null;
  const base = (info.name || 'video.mp4').replace(/\.[^.]+$/, '');
